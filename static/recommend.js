$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = '8aeb8f3a8c438e473b6ee5641e4366f1';
    var title = $('.movie').val();
    if (title=="") {
...
function movie_recs(movie_title,movie_id,my_api_key){
  $.ajax({
    ...
    success: function(recs){
      if(recs=="Sorry! The movie you requested is not in the database. Kindly check the spelling or try with some other movies"){
        $('.fail').css('display','block');
        $('.results').css('display','none');
        $("#loader").delay(500).fadeOut();
      }
